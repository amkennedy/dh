...
if nargin >= 1
    ...
    %if exist(source,'file') == 2
      ...
      % Add you import filter, in this case its sql2gceds() from LNDB source;
      ...
      if gce_valid(data)
         ...
         if ~isempty(data)
             %reorder columns
             cols = name2col(data,{'Date','AIRTEMP_MEAN_150_0_01','INTENSITY_MEAN_150_0_01','DataSetName'}); % check for column to qc
             if ~isempty(cols)
                 [data,msg] = copycols(data, cols);
             end
         end
...
%                 end
                ...
                %add MATLAB serial date column
                %cols = name2col(data,'Year','Month','Day','Hour','Minute');
                [data,msg] = add_datecol(data,[],'Date');
                
%                 [data,msg] = copycol(data,'Date','Date_Str');
%                 
%                 %add MATLAB serial date column, get serial dates for title generation
%                 [data,msg] = add_datecol(data,31,'Date_Str');
                
                if ~isempty(data)
                    %syntax: [s2,msg] = pad_date_gaps(s,datecol,remove_dupes,repl_nondata,min_interval,flag)
                    [data,msg] = pad_date_gaps(data,'Date');
                end
            end
            ...
            if ~isempty(data)
                ...
                interpcols = {'AIRTEMP_MEAN_150_0_01'};
         ...
         %begin elevated qc routine using seperate qc_stats file
            ...
            %structure
            col = name2col(data,{'AIRTEMP_MEAN_150_0_01'}); % check for column to qc
            if ~isempty(col)
                ...
                if ~isempty(data)
                    cols = {'Year','Minute','Second','Month','Day','Hour','FOURLO','FOURHI'};
...
%    else      
% %       msg = [pn_source,' is not present in the MATLAB search path'];      
% %    end
