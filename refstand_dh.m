function [msg,status] = refstand_dh(source,qc_source,template,sitecode,profile,pn_dest,pn_plots,html,email,fn_dest,reprocess)
%Data harvester function template for post-processing streaming
%sensor data retrieved from an SQL data source
%this harvester has been retired, but a new version has not been
%created yet. ps. also learing git on a mac.
%
      ...
      if gce_valid(data)
        ...
        %append data to existing file, if present, updating the title to include the overall date range
         ...
         if ~isempty(data)
            ...
            % this works, but I will occastionally see floating
            % point errors on padded dates.
            col = name2col(data,'Date');
            if ~isempty(col)
         ...
         % to do: at some point, we need to generate monthly plots
         % so we can keep better track of the longer term trends at
         % a glance.
